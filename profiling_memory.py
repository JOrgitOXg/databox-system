from memory_profiler import profile
from app import create_app
from app.config import Config
from firebase_admin import firestore, auth
from datetime import datetime

app = create_app()
db = firestore.client()

@profile
def profile_login():
    email = "jorge.pimentel630@gmail.com"
    password = "jorge664."
    
    try:
        user = auth.get_user_by_email(email)
        user_ref = db.collection('users').where('email', '==', email).limit(1).get()
        
        if user_ref:
            user_data = user_ref[0].to_dict()
            full_name = user_data.get('fullName', user.email.split('@')[0])
        else:
            full_name = user.email.split('@')[0]
            db.collection('users').document(user.uid).set({
                'uid': user.uid,
                'email': user.email,
                'fullName': full_name,
                'createdAt': datetime.now()
            })
        
        db.collection('users').document(user.uid).update({
            'lastLogin': datetime.now()
        })
        
        return True
    except Exception as e:
        print(f"Error en login: {str(e)}")
        return False

@profile
def profile_create_survey():
    try:
        survey_data = {
            'title': "Encuesta de perfilado",
            'description': "Encuesta para pruebas de rendimiento",
            'questions': [
                {'text': "¿Cómo calificarías nuestro servicio?", 'type': "rating"},
                {'text': "¿Qué características te gustaría ver?", 'type': "open"},
                {'text': "¿Recomendarías este servicio?", 'type': "multiple"}
            ],
            'owner': "test_user_id",
            'createdAt': datetime.now(),
            'responses': 0,
            'updatedAt': datetime.now(),
            'is_public': True
        }
        
        doc_ref = db.collection('surveys').add(survey_data)
        print(f"Encuesta creada con ID: {doc_ref[1].id}")
        return True
    except Exception as e:
        print(f"Error creando encuesta: {str(e)}")
        return False

@profile
def profile_process_responses():
    try:
        survey_id = "PAmkaY9eZohrXajCslKy"
        responses = []
        
        for i in range(1, 101):
            response_data = {
                'survey_id': survey_id,
                'user_info': {
                    'name': f"Usuario {i}",
                    'age': 20 + (i % 40),
                    'city': "Ciudad Ejemplo"
                },
                'responses': [
                    {'question_number': 1, 'response': f"Respuesta abierta {i}"},
                    {'question_number': 2, 'response': "si" if i % 3 == 0 else "no"},
                    {'question_number': 3, 'response': str((i % 5) + 1)}
                ],
                'submitted_at': datetime.now(),
                'is_anonymous': True
            }
            responses.append(response_data)
        
        batch = db.batch()
        for resp in responses:
            new_ref = db.collection('survey_responses').document()
            batch.set(new_ref, resp)
        
        batch.commit()
        
        db.collection('surveys').document(survey_id).update({
            'responses': firestore.firestore.SERVER_TIMESTAMP,
            'updatedAt': datetime.now()
        })
        
        return True
    except Exception as e:
        print(f"Error procesando respuestas: {str(e)}")
        return False

@profile
def profile_generate_stats():
    try:
        survey_id = "PAmkaY9eZohrXajCslKy"
        survey_ref = db.collection('surveys').document(survey_id)
        survey_doc = survey_ref.get()
        
        if not survey_doc.exists:
            print("La encuesta no existe.")
            return False
        
        survey = survey_doc.to_dict()
        
        responses = []
        for resp in db.collection('survey_responses').where('survey_id', '==', survey_id).stream():
            resp_data = resp.to_dict()
            clean_responses = []
            for answer in resp_data.get('responses', []):
                clean_answer = {
                    'question_number': int(answer.get('question_number', 0)),
                    'response': str(answer.get('response', ''))
                }
                clean_responses.append(clean_answer)
            resp_data['responses'] = clean_responses
            responses.append(resp_data)

        stats = {'questions': []}
        question_stats = [{} for _ in survey.get('questions', [])]

        for resp in responses:
            for answer in resp.get('responses', []):
                qn = int(answer['question_number']) - 1
                response = answer['response']
                if 0 <= qn < len(question_stats):
                    q_type = survey['questions'][qn]['type']
                    
                    if q_type == 'open':
                        question_stats[qn].setdefault('responses', []).append(response)
                    elif q_type == 'multiple':
                        question_stats[qn].setdefault('counts', {})
                        question_stats[qn]['counts'][response] = question_stats[qn]['counts'].get(response, 0) + 1
                    elif q_type == 'rating':
                        question_stats[qn].setdefault('ratings', [])
                        try:
                            rating = int(response)
                            question_stats[qn]['ratings'].append(rating)
                        except ValueError:
                            continue

        for i, q in enumerate(survey.get('questions', [])):
            q_type = q.get('type')
            result = {'text': q.get('text'), 'type': q_type}

            if q_type == 'open':
                result['responses'] = question_stats[i].get('responses', [])
            elif q_type == 'multiple':
                result['counts'] = question_stats[i].get('counts', {})
            elif q_type == 'rating':
                ratings = question_stats[i].get('ratings', [])
                if ratings:
                    result['average'] = sum(ratings) / len(ratings)
                    result['count'] = len(ratings)
                    result['distribution'] = {str(r): ratings.count(r) for r in range(1, 6)}
                else:
                    result['average'] = 0
                    result['count'] = 0
                    result['distribution'] = {}

            stats['questions'].append(result)

        print("Estadísticas generadas correctamente")
        return stats
    except Exception as e:
        print(f"Error generando estadísticas: {str(e)}")
        return False

def run_profiling():
    print("Iniciando perfilado de memoria...\n")
    
    functions_to_profile = [
        ("Autenticación de usuario", profile_login),
        ("Creación de encuestas", profile_create_survey),
        ("Procesamiento de respuestas", profile_process_responses),
        ("Generación de estadísticas", profile_generate_stats)
    ]
    
    for name, func in functions_to_profile:
        print(f"\n=== Perfilando: {name} ===")
        resultado = func()
        print(f"Resultado: {'Éxito' if resultado else 'Fallo'}")
    
    print("\nPerfilado completado. Para visualización avanzada:")
    print("1. Ejecuta 'python -m memory_profiler profiling_memory.py'")
    print("2. Puedes usar herramientas como memory_profiler o guppy para inspección más detallada.")

if __name__ == '__main__':
    run_profiling()
